...
WITH summarized AS(
  ...
  SELECT card, fare_id,summer_transactions_percent,
    CASE
  ...
  SELECT fare_id,
    Count(CASE WHEN (range = 0) THEN 1 END) AS range_0_50,
    Count(CASE WHEN (range = 1) THEN 1 END) AS range_50_75,
    Count(CASE WHEN (range = 2) THEN 1 END) AS range_75_100,
    Count(CASE WHEN (range = 3) THEN 1 END) AS range_100
  FROM reclassified
  GROUP BY fare_id
), summarized_rows AS (
